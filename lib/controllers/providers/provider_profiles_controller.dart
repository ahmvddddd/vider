...
>((ref) => ProviderProfilesNotifier(ref));

class ProviderProfilesNotifier
    extends StateNotifier<AsyncValue<Map<String, List<dynamic>>>> {
  final Ref ref;
  ProviderProfilesNotifier(this.ref) : super(const AsyncValue.loading()) {
      ...
      if (res.statusCode == 200) {
        ...
        try {
          await FirebaseCrashlytics.instance.recordError(
            ...
            null,
            reason:
                'OSM State coordinates API returned error ${res.statusCode}',
          );
        } catch (e) {
          logger.i("Crashlytics logging failed: $e");
        }
